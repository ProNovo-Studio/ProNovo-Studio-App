...
const generateSessionTitle = (messages: Message[]): string => {
  ...
  return (
    words.join(" ") + (content.length > words.join(" ").length ? "..." : "")
  );
};
    ...
    (set, get) => ({
      ...
      deleteSession: (sessionId) => {
        set((state) => {
          ...
          const updatedRecentIds = state.recentSessionIds.filter(
            (id) => id !== sessionId
          );
          ...
          return {
            ...
            recentSessionIds: updatedRecentIds,
            selectedSessionIds: state.selectedSessionIds.filter(
              (id) => id !== sessionId
            ),
          };
        });
      ...
      importSessions: (jsonData) => {
        try {
          ...
          const importedSessions = importData.sessions.map((session: any) => ({
            ...session,
            id: Math.floor(Math.random() * 1000000), // Generate new IDs to avoid conflicts
            createdAt: new Date(session.createdAt),
            lastModified: new Date(session.lastModified),
            messages: session.messages.map((msg: any) => ({
              ...msg,
              // Remove timestamp, ensure required fields
              id:
              ...
              content: msg.content,
              data: msg.data ?? null,
              thread_id: msg.thread_id || session.id,
            })),
          }));
      ...
      cleanupOldSessions: (retentionDays) => {
        ...
        set((state) => {
          const remainingSessions = state.sessions.filter((session) => {
            const shouldDelete =
              session.lastModified < cutoffDate && !session.metadata.starred;
          ...
          return {
            ...
            activeSessionId: remainingSessions.find(
