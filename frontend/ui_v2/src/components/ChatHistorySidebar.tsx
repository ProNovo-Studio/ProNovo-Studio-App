...
export const ChatHistorySidebar: React.FC = () => {
  ...
  const handleDeleteSelected = () => {
    if (
      confirm(
        `Delete ${selectedSessionIds.length} selected chat session(s)? This cannot be undone.`
      )
    ) {
  ...
  return (
    <>
      ...
      <div
        className={`w-64 bg-white border-r border-gray-200 text-gray-900 flex flex-col h-full flex-shrink-0 animate-in slide-in-from-left duration-300 ${
          isMobile
            ? "fixed left-0 top-0 z-50 md:relative md:z-auto shadow-xl"
            : ""
        }`}
      >
        {/* Header */}
