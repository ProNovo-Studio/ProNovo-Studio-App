...
export const ChatHistoryItem: React.FC<ChatHistoryItemProps> = ({
  ...
  const { switchToSession, toggleSessionSelection, setSidebarCollapsed } =
    useChatHistoryStore();

  const { updateSessionTitle, starSession, duplicateSession, deleteSession } =
    useSessionManagement();
  ...
  const getConversationIcon = () => {
    const firstUserMessage =
      session.messages.find((m) => m.type === "user")?.content.toLowerCase() ||
      "";

    if (
      firstUserMessage.includes("code") ||
      firstUserMessage.includes("function") ||
      firstUserMessage.includes("debug")
    ) {
      return <Code className="w-4 h-4 text-green-400" />;
    } else if (
      firstUserMessage.includes("search") ||
      firstUserMessage.includes("find")
    ) {
      return <Search className="w-4 h-4 text-blue-400" />;
    } else if (
      firstUserMessage.includes("show") ||
      firstUserMessage.includes("display") ||
      firstUserMessage.includes("visualize")
    ) {
